...
* 📝 **Pure-Text Second Brain Integration:** The entire knowledge architecture operates strictly in **human-readable plain text (`.org`, `.md`, `.bib`, `.py`)** stored in a local "Second Brain" (`Emacs Denote`, `tasks.org`, `citar`, `references.bib`). No proprietary binary databases or SaaS lock-ins exist: notes, task agendas, bibliographies, and system configurations are 100% version-controlled via Git and guaranteed to remain readable across decades.
* 🌌 **Universal & Unlimited Domain Applicability:** `AGY-Bridge` is not restricted to a single specialized task. It is a **universal research and computing platform** adaptable to any academic domain (Physics, Chemistry, Biomedicina, History, Education, Engineering), administrative workflow, or software development project.
* 🛠️ **Live Self-Modification & Total Customization:** The system dynamically evolves through natural language during live usage. From customizing Linux desktop UI themes (Rofi Tokyo-Night grid, i3status VPN indicators, window border rules) and volume OSD scripts to adding paginated web search bars and interactive PTY terminal modals, the agent customizes both itself and the workstation on the fly.
